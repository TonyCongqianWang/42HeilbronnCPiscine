...
/*   Updated: 2025/01/28 17:18:25 by towang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		try_set_grid_val(t_puzzle *grid, int cell_idx, int val)
{
	...
	if (!check_constraints(grid, cell_idx))
	{
		...
		unset_set_grid_val(grid, cell_idx);
		return (0);
	}
}
...
int	check_constraints(t_puzzle *puzzle, int insert_idx)
{
	...
	while (rel_idx < 2)
	{
		...
		r01_set_active_constraint(puzzle, abs_idx);
		if(!r01_check_active_constr(puzzle))
			return (0);
		reverse_direction(&puzzle->constr_state);
		if(!r01_check_active_constr(puzzle))
			return (0);
		rel_idx++;
	}
