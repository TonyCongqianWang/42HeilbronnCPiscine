...
/*   Updated: 2025/01/25 22:34:35 by towang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	r01_check_constr(t_r01_puzzle *puzzle, t_r01_constraint *constraint)
{
	...
	while (constr_sub_idx < constraint->size)
	{
		...
		r01_update_constr(puzzle->grid_vals[grid_idx], constraint);
...
int	r01_check_constr_violation(t_r01_constraint *constr, int sub_idx)
{
	...
	if (constr->n_seen > constr->target_val)
		return (1);
...
void	r01_update_constr(int val, t_r01_constraint *constr)
{
	if (val == 0)
	{
		constr->n_unset++;
